 ...
 *
 * 1. REQUIRED
 *      가장 많이 사용하는 기본 설정. 기존 트렌젝션이 없으면 생성하고, 있으면 참여한다.
 *      트렌젝션이 필수라는 의미
 *      기존 트렌젝션 없음: 새로운 트렌젝션을 생성
 *      기존 트렌젝션 있음: 기존의 트렌젝션에 참여
 *
 * 2. REQUIRES_NEW
 *      항상 새로운 트렌젝션을 생성한다.
 *      기존 트렌젝션 없음: 새로운 트렌젝션을 생성
 *      기존 트렌젝션 있음: 새로운 트렌젝션을 생성
 *
 * 3. SUPPORT
 *      트렌젝션을 `지원`한다는 의미. 기존 트렌젝션이 있으면 사용, 없으면 트렌젝션 걸지 않은 상태로 진행
 *      기존 트렌젝션 없음: 트렌젝션을 걸지 않은 상태로 진행
 *      기존 트렌젝션 있음: 기존 트렌젝션에 참여
 *
 * 4. NOT_SUPPORT
 *      트렌젝션을 `지원`하지 않는다는 의미.
 *      기존 트렌젝션 없음: 트렌젝션을 걸지 않은 상태로 진행
 *      기존 트렌젝션 있음: 트렌젝션을 걸지 않은 상태로 진행 (기존 트렌젝션은 suspend)
 *
 * 5. MANDATORY
 *      의무사항이다, 기존 트렌젝션이 반드시 있어야 한다. 기존 트렌젝션이 없으면 예외가 발생
 *      기존 트렌젝션 없음: IllegalTransactionStateException 발생
 *      기존 트렌젝션 있음: 기존 트렌젝션에 참여
 *
 * 6. NEVER
 *      트렌젝션을 전혀 사용하지 않는다는 의미. 기존 트렌젝션이 있으면 예외가 발생한다.
 *      기존 트렌젝션도 사용하지 않는다는 강한 부정의 의미
 *      기존 트렌젝션 없음: 트렌젝션을 걸지 않은 상태로 진행
 *      기존 트렌젝션 있음: IllegalTransactionStateException 발생
 *
 * 7. NESTED
 *      기존 트렌젝션 없음: 새로운 트렌젝션을 생성
 *      기존 트렌젝션 있음: 중첩 트렌젝션을 생성
 *          중첩 트렌젝션은 외부 트렌젝션에 영향을 받는다. 하지만 외부 트렌젝션은 중첩 트렌젝션의 영향을 받지 않는다.
 *          중첩 트렌젝션 롤백 이더라도, 외부 트렌젝션은 커밋 가능
 *          외부 트렌젝션이 롤백이라면, 중첩 트렌젝션도 롤백
 *      참고:
 *          JDBC `savepoint`기능을 사용한다, DB 드라이버에서 해당 기능을 지원하는지 확인해야한다.
 *          중첩 트렌젝션은 JPA에서는 사용할 수 없다.
 *
 *  `isolation`, `timeout`, `readOnly`는 트렌젝션이 처음 시작될때만 적용된다.
 *  기존의 트렌젝션에 참여하는 경우, 후에 바꿔도 적용되지 않는다.
 *  REQUIRED, REQUIRES_NEW를 통해 새 트렌젝션이 시작 되는 시점에만 적용된다.
 *
 *      @Transactional(readOnly = true) // readOnly = true
 *      fun outer() {
 *          inner() // 여기 호출 시 readOnly = false를 줘도 무시됨
 *      }
 *
 *      @Transactional(readOnly = false)
 *      fun inner() {
 *          // 실제로는 readOnly = true, 기존 트렌젝션이 참여한것이기 때문에.
 *      }
 * */
